...
def RandomStair(t,stair_step,values): #vetor que gera uma sequencia aleatoria de valores, onde t eh um vetor de referencia, stair_step eh o numero de iteracoes que se mantem um valor
...
#        break
stair_step_ctrl = 200
Tref_plot[:First_step_end] = 35 + 30*signal.square(2 * np.pi * 0.0005 * t[:First_step_end])
Tref_plot[First_step_end:] = RandomStair(Tref_plot[First_step_end:],stair_step_ctrl,[30,40,50,60,55,45,40,35,30,32,33,49])
...
p6 = sub[2].plot(iterations,Training_Error_plot,label = 'erro de treinamento')
